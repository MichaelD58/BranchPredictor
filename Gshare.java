...
class GShare {
    ...
    static void reader(File file, int tableSize){
        ...
        if(taken.size() > 0){
            System.out.println(filenameArray[2] + ": " + df.format(gShare(branch, taken, tableSize)) + "%");//Output of the strategy method
            fileCount++;
        }
    ...
    static double gShare(ArrayList<String> branch, ArrayList<Integer> taken, int tableSize){
        ...
        String global = "0";//Global history
        ...
        for(int i = 0; i < taken.size(); i++){
            ...
            // int globalSize = 4;
            // int globalSize = 8;
            int globalSize = sigBits;//Size of the global history is set

            //If global is longer than set length allows, cut it
            if (global.length() > globalSize) {
                global = global.substring(global.length() - globalSize);
            }

            int a = Integer.parseInt(binary, 2);//Binary address is converted to integer for XOR
            int b = Integer.parseInt(global, 2);//Global is converted to integer for XOR
            int index = a ^ b;//XOR is carried out
            ...
            if(buffer[index].equals("00")){
                if(taken.get(i) == 1){
                    buffer[index] = "01";//Incorrect prediction
                }else{
            ...
            global += taken.get(i);//Actual branch taken/non-taken is added to the global history
        }
